...
`;const Ce={solar:{x:250,y:46,radius:36},inverter:{x:250,y:170,radius:34},battery:{x:74,y:170,radius:42},home:{x:426,y:170,radius:38},grid:{x:250,y:294,radius:36}};function Ee(e,t=1){return Math.abs(e)>=1e3?`${(e/1e3).toFixed(t)} kW`:`${Math.round(e)} W`}function Se(e){const{config:t,solarWatts:i,batteryWatts:r,gridWatts:a,loadWatts:o,batterySoc:n,operatingMode:s,isFault:l,onNodeClick:c}=e,d=t.max_power||4200,p=!1!==t.use_log_flow_model,h=t.min_flow_rate||.75,g=t.max_flow_rate||6,_=function(e,t,i,r,a){const o=[],n=a?.solar||be,s=a?.battery_charge||fe,l=a?.battery_discharge||ve,c=a?.grid_import||xe,d=a?.grid_export||we,p=a?.home||$e,{solar:h,inverter:g,battery:_,home:u,grid:y}=Ce,m=Math.max(0,e);m>=5&&o.push({id:"flow-solar-inverter",d:`M ${h.x} ${h.y+h.radius} L ${g.x} ${g.y-g.radius}`,watts:m,color:n,reversed:!1});const b=Math.max(0,r);b>=5&&o.push({id:"flow-inverter-home",d:`M ${g.x+g.radius} ${g.y} L ${u.x-u.radius} ${u.y}`,watts:b,color:p,reversed:!1});const f=Math.abs(t);if(f>=5){const e=t>0;o.push({id:"flow-battery-inverter",d:`M ${_.x+_.radius} ${_.y} L ${g.x-g.radius} ${g.y}`,watts:f,color:e?s:l,reversed:e})}const v=Math.abs(i);if(v>=5){const e=i>0;o.push({id:"flow-grid-inverter",d:`M ${g.x} ${g.y+g.radius} L ${y.x} ${y.y-y.radius}`,watts:v,color:e?c:d,reversed:e})}return o}(i,r,a,o,t.colors),{solar:u,inverter:y,battery:m,home:b,grid:f}=Ce,v=Math.max(0,Math.min(100,isNaN(n)?0:n)),x=2*Math.PI*32,w=x*(1-v/100),$=v>50?"#10b981":v>20?"#f59e0b":"#ef4444",k=r>20?"Charge":r<-20?"Discharge":"Standby";return W`
    <div class="power-flow-container">
      ...
      <svg class="flow-svg" viewBox="0 0 500 340" preserveAspectRatio="xMidYMid meet">
        ...
        <path
          class="pipe-track"
          d="M ${u.x} ${u.y+u.radius} L ${y.x} ${y.y-y.radius}"
        />
        <path
          class="pipe-track"
          d="M ${m.x+m.radius} ${m.y} L ${y.x-y.radius} ${y.y}"
        />
        <path
          class="pipe-track"
          d="M ${y.x+y.radius} ${y.y} L ${b.x-b.radius} ${b.y}"
        />
        <path
          class="pipe-track"
          d="M ${y.x} ${y.y+y.radius} L ${f.x} ${f.y-f.radius}"
        />

        <!-- Active animated flow lines -->
        ${_.map(e=>{const t=function(e,t=4200,i=.75,r=6,a=!0){const o=Math.abs(e);if(o<5)return 0;const n=Math.min(o,t);let s;s=a?Math.log10(n+1)/Math.log10(t+1):n/t;const l=r-s*(r-i);return Math.max(i,Math.min(r,Number(l.toFixed(2))))}(e.watts,d,h,g,p),i=function(e,t=4200){const i=Math.min(Math.abs(e)/t,1);return Number((2.5+3*i).toFixed(1))}(e.watts,d);return j`
            <path
              id="${e.id}"
      ...
      <div class="flow-nodes-layer">
        ...
        <div
          ...
          style="left: ${u.x/500*100}%; top: ${u.y/340*100}%;"
        ...
        <div
          ...
          style="left: ${m.x/500*100}%; top: ${m.y/340*100}%;"
        ...
        <div
          ...
          style="left: ${y.x/500*100}%; top: ${y.y/340*100}%;"
        ...
        <div
          ...
          style="left: ${b.x/500*100}%; top: ${b.y/340*100}%;"
        ...
        <div
          ...
          style="left: ${f.x/500*100}%; top: ${f.y/340*100}%;"
  ...
  `}({config:this._config,operatingMode:o,temperature:n,isCharging:s,isFault:l,faultCode:c,isWarning:d,warningCode:p,onChipClick:e=>this._handleEntityClick(e)}):""}
  ...
  `}({safetyConfig:w,isFault:l,faultCode:c,isWarning:d,warningCode:p,onAlertClick:e=>this._handleEntityClick(e)}):""}

        <!-- 3. Power Flow Animated Diagram -->
        ${!1!==this._config.show_flow?Se({hass:this._hass,config:this._config,solarWatts:e,batteryWatts:t,gridWatts:i,loadWatts:r,batterySoc:a,operatingMode:"N/A"!==o?o:"Inverter",isFault:l,onNodeClick:e=>this._handleEntityClick(e)}):""}

        <!-- 4. Electrical Telemetry & Self-Sufficiency KPI -->
        ${!1!==this._config.show_params?Me({config:this._config,batteryVoltage:h,batteryCurrent:g,gridVoltage:_,gridImportEnergyToday:u,loadEnergyToday:y,historyMap:this._historyMap,onParamClick:e=>this._handleEntityClick(e)}):""}
